...
def sample(
    ...
    if num_numerical_features != 0:
        ...
        X_num_pd=pd.DataFrame(X_num)
        X_num_pd.to_csv(os.path.join(parent_dir, 'X_gen.csv'))
    if num_numerical_features < X_gen.shape[1]:
        np.save(os.path.join(parent_dir, 'X_cat_train'), X_cat)
    np.save(os.path.join(parent_dir, 'y_train'), y_gen)
    y_gen_pd=pd.DataFrame(y_gen)
    y_gen_pd.to_csv(os.path.join(parent_dir,"y_gen.csv"))
